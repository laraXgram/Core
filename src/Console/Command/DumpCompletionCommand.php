...
EOH
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!file_exists($completionFile)) {
            ...
            if ($shell) {
                $output->writeln(\sprintf('<error>Detected shell "%s", which is not supported by LaraGram shell completion (supported shells: "%s").</>', $shell, implode('", "', $supportedShells)));
            } else {
                $output->writeln(\sprintf('<error>Shell not detected, LaraGram shell completion only supports "%s").</>', implode('", "', $supportedShells)));
            }
