...
use LaraGram\Console\Helper\TableCellStyle;
...
use LaraGram\Console\Prompts\Convertor\HtmlRenderer;
...
final class TableRenderer
{
        ...
        foreach ($node->getChildNodes() as $child) {
            if ($child->isName('th') || $child->isName('td')) {
                ...
                $row[] = new TableCell(
                    ...
                    (string) Convertor::span($text, $class),
                    [
    ...
    private function parseCellStyle(string $styles, string $align = TableCellStyle::DEFAULT_ALIGN): TableCellStyle
    {
        ...
        $element = Convertor::span('%s', $styles);
    ...
    {
        return (string) Convertor::span(' %s ', $node->getClassAttribute());
    }
}
