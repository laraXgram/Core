...
class Temple8Compiler extends Compiler implements CompilerInterface
{
    ...
    /**
     * Append the method call to the compiled string.
     *
     * @param  string  $contents
     * @return string
     */
    ...
    /**
     * Compile the given Temple8 template contents.
     *
     * @param  string  $value
     * @return string
     */
    public function compileString($value)
    {
        ...
        // First we will compile the Temple8 component tags. This is a precompile style
        // step which compiles the component Temple8 tags into @component directives
        // that may be used by Temple8. Then we should call any other precompilers.
        ...
        if (! empty($this->echoHandlers)) {
            $result = $this->addTemple8CompilerVariable($result);
        }
    ...
    /**
     * Evaluate and render a Temple8 string to HTML.
     *
     * @param  string  $string
     * @param  array  $data
     * @param  bool  $deleteCachedTemplate
     * @return string
     */
    public static function render($string, $data = [], $deleteCachedTemplate = false)
    {
        ...
        $template = Container::getInstance()
            ->make(TemplateFactory::class)
            ->make($component->resolveTemplate(), $data);

        return tap($template->render(), function () use ($template, $deleteCachedTemplate) {
            if ($deleteCachedTemplate) {
     ...
     *
     * @param  \LaraGram\Template\Component  $component
     * @return string
     */
    public static function renderComponent(Component $component)
    {
        ...
        $template = value($component->resolveTemplate(), $data);
    ...
    /**
     * Compile Temple8 statements that start with "@".
     *
    ...
    protected function compileStatements($template)
    {
        ...
        for ($i = 0; isset($matches[0][$i]); $i++) {
            ...
            if ($compiled = $this->tryCompileAllowedDirective($match)) {
    ...
    /**
     * Compile a single Temple8 @ statement.
     *
     * @param  array  $match
     * @return string
     */
    ...
    /**
     * Register a custom Temple8 compiler.
     *
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php if (\LaraGram\Support\Facades\Temple8::check('{$name}', {$expression})): ?>"
                    : "<?php if (\LaraGram\Support\Facades\Temple8::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\LaraGram\Support\Facades\Temple8::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\LaraGram\Support\Facades\Temple8::check('{$name}')): ?>";
        });
    ...
    public function component($class, $alias = null, $prefix = '')
    {
        ...
        if (is_null($alias)) {
            $alias = str_contains($class, '\\Template\\Components\\')
                            ? (new Collection(explode('\\', Str::after($class, '\\Template\\Components\\'))))->map(function ($segment) {
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php echo \$__env->make('{$path}', {$expression}, \LaraGram\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
    ...
    public function directive($name, callable $handler, bool $bind = false)
    {
        ...
        $this->customDirectives[$name] = $bind ? $handler->bindTo($this, Temple8Compiler::class) : $handler;
    }
