...
class ArrayStore extends TaggableStore implements CanFlushLocks, LockProvider
{
     ...
     * The array of locks.
     *
     * @var array<string, array{owner: ?string, expiresAt: ?\LaraGram\Support\Tempora}>
     */
    public $locks = [];
    ...
    public function get($key)
    {
        ...
        if ($expiresAt !== 0 && (Tempora::now()->getPreciseTimestamp(3) / 1000) >= $expiresAt) {
            $this->forget($key);
    ...
    {
        return $seconds > 0 ? (Tempora::now()->getPreciseTimestamp(3) / 1000) + $seconds : 0;
    }
