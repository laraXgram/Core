...
use LaraGram\Foundation\Events\MaintenanceModeEnabled;
use LaraGram\Foundation\Exceptions\RegisterErrorViewPaths;
...
class DownCommand extends Command
{
    ...
    {
        try {
            $wasAlreadyDown = $this->laragram->maintenanceMode()->active();

            $downFilePayload = $this->getDownFilePayload();

            $this->laragram->maintenanceMode()->activate($downFilePayload);
            ...
            $this->laragram->get('events')->dispatch(new MaintenanceModeEnabled());
    ...
    {
        try {
            return $this->laragram->make(AppPreventRequestsDuringMaintenance::class)->getExcludedPaths();
        } catch (Throwable) {
            try {
                return $this->laragram->make(PreventRequestsDuringMaintenance::class)->getExcludedPaths();
            } catch (Throwable) {
                return [];
            }
        }
