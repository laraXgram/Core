...
use LaraGram\Support\Env\Parser\Lines;
use Exception;
...
class EnvironmentDecryptCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $encryptedFile = ($this->option('env')
                ? Str::finish($this->laragram->environmentPath(), DIRECTORY_SEPARATOR).'.env.'.$this->option('env')
                : $this->laragram->environmentFilePath()).'.encrypted';
     ...
     *
     * @param  string  $contents
     * @param  \LaraGram\Encryption\Encrypter  $encrypter
     * @return string
     */
    ...
    {
        $path = Str::finish($this->option('path') ?: $this->laragram->environmentPath(), DIRECTORY_SEPARATOR);
