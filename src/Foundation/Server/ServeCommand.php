...
class ServeCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (($_ENV['DEVELOPMENT_SERVER_IP'] == null && $this->options['host'] == null) || ($_ENV['DEVELOPMENT_SERVER_PORT'] == null && $this->options['port'] == null)) Console::output()->failed("DEVELOPMENT_SERVER_IP or DEVELOPMENT_SERVER_PORT not set!", exit: true);
        ...
        if ($this->getOption('apiserver') == 'apiserver') {
            ...
            if ($_ENV['BOT_API_SERVER_IP'] == null || $_ENV['BOT_API_SERVER_PORT'] == null) Console::output()->failed("BOT_API_SERVER_IP or BOT_API_SERVER_PORT not set!", exit: true);

            $dir = $_ENV['BOT_API_SERVER_DIR'] ?? app('path.storage') . '/API-Server';

            if (!file_exists($dir)) mkdir($dir, recursive: true);

            Console::output()->success("Starting API Server on {$_ENV['BOT_API_SERVER_IP']}:{$_ENV['BOT_API_SERVER_PORT']}");
