...
class BaseListen implements \Serializable
{
    ...
    private ?CompiledListen $compiled = null;
     ...
     * Available options:
     *
     *  * compiler_class: A class name able to compile this listen instance (ListenCompiler by default)
     ...
     * @param string|null               $condition    A condition that should evaluate to true for the listen to match
     */
    public function __construct(string $pattern, array $defaults = [], array $requirements = [], array $options = [], string|array $methods = [], ?string $condition = '')
    {
    ...
    /**
     * Returns the uppercased methods this listen is restricted to.
    ...
    /**
     * Sets the HTTP methods (e.g. 'POST') this listen is restricted to.
     * So an empty array means that any method is allowed.
     *
    ...
    {
        $this->options = [
            'compiler_class' => ListenCompiler::class,
        ];
     ...
     *
     * @see ListenCompiler which is responsible for the compilation process
     */
    public function compile(): CompiledListen
    {
    ...
    private function extractInlineDefaultsAndRequirements(string $pattern): string
    {
        ...
        $mapping = $this->getDefault('_listen_mapping') ?? [];
        ...
        if ($mapping) {
            $this->setDefault('_listen_mapping', $mapping);
        }
    ...
    private function sanitizeRequirement(string $key, string $regex): string
    {
        ...
        if ('' === $regex) {
            throw new \InvalidArgumentException(\sprintf('Listening requirement for "%s" cannot be empty.', $key));
        }
    ...
    {
        return isset($this->defaults['_locale']) && isset($this->defaults['_canonical_listen']) && ($this->requirements['_locale'] ?? null) === preg_quote($this->defaults['_locale']);
    }
}
