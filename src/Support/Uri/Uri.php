...
use Deprecated;
use finfo;
use LaraGram\Support\Uri\Contracts\Conditionable;
use LaraGram\Support\Uri\Contracts\FragmentDirective;
use LaraGram\Support\Uri\Contracts\Transformable;
use LaraGram\Support\Uri\Contracts\UriComponentInterface;
use LaraGram\Support\Uri\Contracts\UriException;
use LaraGram\Support\Uri\Contracts\UriInterface;
use LaraGram\Support\Uri\Exceptions\MissingFeature;
use LaraGram\Support\Uri\Exceptions\SyntaxError;
use LaraGram\Support\Uri\Idna\Converter as IdnaConverter;
use LaraGram\Support\Uri\IPv4\Converter as IPv4Converter;
use LaraGram\Support\Uri\IPv6\Converter as IPv6Converter;
use LaraGram\Http\Factory\UriInterface as Psr7UriInterface;
...
final class Uri implements Conditionable, UriInterface, Transformable
{
    ...
    public static function fromTemplate(BackedEnum|UriTemplate|Stringable|string $template, iterable $variables = []): self
    {
        return match (true) {
            ...
            $template instanceof Template => self::new($template->expand($variables)),
            default => self::new(Template::new($template)->expand($variables)),
        };
    }
