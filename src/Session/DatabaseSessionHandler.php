...
use LaraGram\Database\QueryException;
...
class DatabaseSessionHandler implements ExistenceAwareInterface, SessionHandlerInterface
{
    ...
    protected function expired($session)
    {
        return isset($session->last_activity) &&
            $session->last_activity < Tempora::now()->subMinutes($this->minutes)->getTimestamp();
    }
    ...
    {
        if ($this->container->bound('http.request')) {
    ...
    {
        return $this->container->make('http.request')->ip();
    }
    ...
    {
        return mb_substr(mb_convert_encoding((string) $this->container->make('http.request')->header('User-Agent'), 'UTF-8'), 0, 500);
    }
     ...
     *
     * @param  \LaraGram\Contracts\Foundation\Application  $container
     * @return $this
     */
