...
use LaraGram\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->laragram['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->laragram['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->laragram['queue.failer']->forget($id);
            }
        }
    ...
    {
        $queue = $this->laragram['queue']->connection($job->connection);

        $this->laragram['queue']->connection($job->connection)->pushRaw(
    ...
    protected function getInstanceFromPayload($payload)
    {
        ...
        if ($this->laragram->bound(Encrypter::class)) {
            return unserialize($this->laragram->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
