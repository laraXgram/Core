...
use LaraGram\Database\DetectsLostConnections;
use LaraGram\Queue\Events\JobAttempted;
use LaraGram\Queue\Events\JobExceptionOccurred;
use LaraGram\Queue\Events\JobPopped;
use LaraGram\Queue\Events\JobPopping;
...
use LaraGram\Queue\Events\JobTimedOut;
use LaraGram\Queue\Events\Looping;
use LaraGram\Queue\Events\WorkerIdle;
use LaraGram\Queue\Events\WorkerInterrupted;
use LaraGram\Queue\Events\WorkerPausing;
use LaraGram\Queue\Events\WorkerResuming;
use LaraGram\Queue\Events\WorkerStarting;
...
class Worker
{
     ...
     *
     * @param  \LaraGram\Contracts\Queue\Queue  $connection
     * @param  string  $queue
    ...
    protected function markJobAsFailedIfAlreadyExceedsMaxAttempts($connectionName, $job, $maxTries)
    {
        ...
        if ($retryUntil && Tempora::now()->getTimestamp() <= $retryUntil) {
            return;
        }
    ...
    protected function markJobAsFailedIfWillExceedMaxAttempts($connectionName, $job, $maxTries, Throwable $e)
    {
        ...
        if ($job->retryUntil() && $job->retryUntil() <= Tempora::now()->getTimestamp()) {
            $this->failJob($job, $e);
        }
    ...
    protected function markJobAsFailedIfWillExceedMaxExceptions($connectionName, $job, Throwable $e)
    {
        ...
        if (! $this->cache->get('job-exceptions:'.$uuid)) {
            $this->cache->put('job-exceptions:'.$uuid, 0, Tempora::now()->addDay());
        }
    ...
    protected function getTimestampOfLastQueueRestart()
    {
        ...
        if ($this->cache) {
            return $this->cache->get('LaraGram:queue:restart');
        }
    }
     ...
     * @param  string|null  $queue
     * @param  \LaraGram\Queue\WorkerOptions|null  $options
     * @return void
     */
     ...
     * @param  int  $status
     * @param  \LaraGram\Queue\WorkerOptions|null  $options
     * @param  \LaraGram\Queue\WorkerStopReason|null  $reason
     * @return never
     */
     ...
     *
     * @return \LaraGram\Contracts\Queue\Factory
     */
