    ...
    public function handle($request, Closure $next, $options = [])
    {
        ...
        if (isset($options['last_modified'])) {
            if (is_numeric($options['last_modified'])) {
                $options['last_modified'] = Tempora::createFromTimestamp($options['last_modified'], date_default_timezone_get());
            } else {
                $options['last_modified'] = Tempora::parse($options['last_modified']);
            }
        }
