...
use Closure;
use Exception;
use LaraGram\Http\Client\Core\Client;
use LaraGram\Http\Client\Core\Cookie\CookieJar;
...
use LaraGram\Http\Client\Core\HandlerStack;
...
use LaraGram\Http\Client\Core\UriTemplate;
use LaraGram\Contracts\Support\Arrayable;
use LaraGram\Http\Client\Events\ConnectionFailed;
use LaraGram\Http\Client\Events\RequestSending;
use LaraGram\Http\Client\Events\ResponseReceived;
use LaraGram\Http\Client\Promises\FluentPromise;
...
class PendingRequest
{
     ...
     *
     * @var \LaraGram\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \LaraGram\Http\Client\Core\Client
     */
    protected $client;
     ...
     *
     * @var \LaraGram\Http\Factory\StreamInterface|string
     */
     ...
     *
     * @var \LaraGram\Http\Client\Core\TransferStats
     */
    ...
    /**
     * The callbacks that should execute after the LaraGram Response is built.
     *
     * @var \LaraGram\Support\Collection<int, (callable(\LaraGram\Http\Client\Response, \LaraGram\Http\Client\Request): \LaraGram\Http\Client\Response|null)>
     */
     ...
     *
     * @var \LaraGram\Support\Collection|null
     */
     ...
     *
     * @var \LaraGram\Http\Client\Promises\PromiseInterface
     */
    protected $promise;
     ...
     *
     * @var \LaraGram\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \LaraGram\Http\Factory\StreamInterface|string  $content
     ...
     *
     * @param  (callable(\LaraGram\Http\Client\Response, \LaraGram\Http\Client\Request): \LaraGram\Http\Client\Response|null)  $callback
     * @return $this
     */
     ...
     * @return \LaraGram\Http\Client\Response|\LaraGram\Http\Client\Promises\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \LaraGram\Http\Client\Response : \LaraGram\Http\Client\Promises\PromiseInterface)
     *
     * @throws \LaraGram\Http\Client\ConnectionException
     */
     ...
     *
     * @param  string  $url
     * @param  array|\JsonSerializable|\LaraGram\Contracts\Support\Arrayable  $data
     * @return \LaraGram\Http\Client\Response|\LaraGram\Http\Client\Promises\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \LaraGram\Http\Client\Response : \LaraGram\Http\Client\Promises\PromiseInterface)
     *
     ...
     *
     * @param  (callable(\LaraGram\Http\Client\Pool): mixed)  $callback
     * @param  non-negative-int|null  $concurrency
     * @return array<array-key, \LaraGram\Http\Client\Response|\Throwable>
     */
     ...
     *
     * @param  callable  $callback
     * @return \LaraGram\Http\Client\Batch
     */
     ...
     * @param  string  $url
     * @param  array  $options
     * @return \LaraGram\Http\Client\Response|\LaraGram\Http\Client\Promises\LazyPromise
     *
     * @phpstan-return (TAsync is false ? \LaraGram\Http\Client\Response : \LaraGram\Http\Client\Promises\LazyPromise)
     *
     * @throws \Exception
     * @throws \LaraGram\Http\Client\ConnectionException
     */
     ...
     *
     * @param  \LaraGram\Http\Client\Response|\Throwable  $response
     * @param  string  $method
     * @param  string  $url
     * @param  array  $options
     * @param  int  $attempt
     ...
     * @param  string  $url
     * @param  array  $options
     * @return \LaraGram\Http\Factory\MessageInterface|\LaraGram\Http\Client\Promises\PromiseInterface
     *
     * @throws \Exception
     */
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $laragramData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $laragramData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($laragramData) && $method === 'GET' && $urlString->contains('?')) {
            $laragramData = (string) $urlString->after('?');
        }

        if (is_string($laragramData)) {
            parse_str($laragramData, $parsedData);

            $laragramData = is_array($parsedData) ? $parsedData : [];
        }

        if ($laragramData instanceof JsonSerializable) {
            $laragramData = $laragramData->jsonSerialize();
        }

        if (is_array($laragramData) && $this->bodyFormat === 'multipart') {
            return $this->normalizeMultipartOption($laragramData);
        }

        return is_array($laragramData) ? $laragramData : [];
    }
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $key => $item) {
                $value[$key] = match (true) {
                    ...
                    default => throw new InvalidArgumentException('HTTP header values must be scalar, null, LaraGram Stringable, or arrays of scalar, null, or LaraGram Stringable values.'),
                };
            }
            ...
            if (is_array($part) && isset($part['headers']) && is_array($part['headers'])) {
                foreach ($part['headers'] as $name => $value) {
                    $multipart[$index]['headers'][$name] = match (true) {
                        ...
                        is_scalar($value) => $this->normalizeScalarString($value),
                        $value instanceof Stringable => $value->toString(),
                        default => throw new InvalidArgumentException('Multipart header values must be scalar, null, or LaraGram Stringable.'),
                    };
                }
            }
        }
    ...
    protected function ensureValidRequestBody($body): void
    {
        if (! is_string($body) && ! is_null($body) && ! is_resource($body) && ! $body instanceof StreamInterface) {
            throw new InvalidArgumentException('HTTP request body must be a string, resource, LaraGram\Http\Factory\StreamInterface, or null.');
        }
    }
     ...
     * @return \LaraGram\Http\Client\Response
     */
     ...
     *
     * @return \LaraGram\Http\Client\Core\Client
     */
     ...
     *
     * @param  \LaraGram\Http\Client\Core\HandlerStack  $handlerStack
     * @return \LaraGram\Http\Client\Core\Client
     */
     ...
     *
     * @return \LaraGram\Http\Client\Core\HandlerStack
     */
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))
                            ->withData($options['laragram_data'] ?? [])
     ...
     * @return \Closure
     *
     * @throws \LaraGram\Http\Client\StrayRequestException
     */
     ...
     *
     * @param  \LaraGram\Http\Factory\RequestInterface  $request
     * @param  array  $options
     * @return \LaraGram\Http\Factory\RequestInterface
     */
     ...
     *
     * @param  \LaraGram\Http\Factory\MessageInterface  $response
     * @return Response
     */
    protected function newResponse($response)
    {
        return tap(new Response($response), function (Response $laragramResponse) {
            if ($this->truncateExceptionsAt === null) {
                return;
            }

            $this->truncateExceptionsAt === false
                ? $laragramResponse->dontTruncateExceptions()
                : $laragramResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
     ...
     *
     * @return \LaraGram\Http\Client\Promises\PromiseInterface|null
     */
     ...
     *
     * @param  \LaraGram\Http\Client\Core\Exceptions\ConnectException  $e
     * @return void
     *
     * @throws \LaraGram\Http\Client\ConnectionException
     */
     ...
     *
     * @param  \LaraGram\Http\Client\Core\Exceptions\RequestException  $e
     * @return void
     *
     * @throws \LaraGram\Http\Client\ConnectionException
     */
     ...
     * @return void
     *
     * @throws \LaraGram\Http\Client\RequestException
     * @throws \LaraGram\Http\Client\ConnectionException
     */
     ...
     *
     * @param  \LaraGram\Http\Client\Core\Client  $client
     * @return $this
     */
