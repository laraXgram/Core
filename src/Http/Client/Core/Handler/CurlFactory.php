...
namespace LaraGram\Http\Client\Core\Handler;
...
use LaraGram\Http\Client\Promises\FulfilledPromise;
use LaraGram\Http\Client\Promises\PromiseInterface;
use LaraGram\Http\Client\Core\LazyOpenStream;
use LaraGram\Http\Client\Core\Uri;
use LaraGram\Http\Client\Core\TransferStats;
use LaraGram\Http\Client\Core\TransportSharing;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    public function create(RequestInterface $request, array $options): EasyHandle
    {
        ...
        if ('' === $protocolVersion) {
            ...
            $protocolVersion = '1.1';
            $request = \LaraGram\Http\Client\Core\PsrUtils::modifyRequest($request, ['version' => $protocolVersion]);
        }
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \LaraGram\Http\Client\Core\PsrUtils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri, ?string $proxy = null): string
    {
        ...
        $redactedUriString = \LaraGram\Http\Client\Core\PsrUtils::redactUserInfo($baseUri)->__toString();
    ...
    private static function redactProxyUserInfo(string $error, ?string $proxy): string
    {
        ...
        try {
            ...
            $redactedUserInfo = \LaraGram\Http\Client\Core\PsrUtils::redactUserInfo($proxyUri)->getUserInfo();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \LaraGram\Http\Client\Core\PsrUtils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \LaraGram\Http\Client\Core\PsrUtils::streamFor($sink);
