...
use LaraGram\Http\Client\Core\Response;
...
class StreamHandler
{
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, ?float $startTime): PromiseInterface
    {
        ...
        $stream = \LaraGram\Http\Client\Core\PsrUtils::streamFor($stream);
        $sink = $stream;
    ...
    private function createSink(StreamInterface $stream, array $options): StreamInterface
    {
        ...
        $sink = $options['sink'] ?? \LaraGram\Http\Client\Core\PsrUtils::tryFopen('php://temp', 'r+');

        return \is_string($sink) ? new \LaraGram\Http\Client\Core\LazyOpenStream($sink, 'w+') : \LaraGram\Http\Client\Core\PsrUtils::streamFor($sink);
    }
        ...
        if (isset($options['decode_content']) && $options['decode_content'] !== false) {
            ...
            if (isset($normalizedKeys['content-encoding'])) {
                ...
                if ($encoding[0] === 'gzip' || $encoding[0] === 'deflate') {
                    $stream = new \LaraGram\Http\Client\Core\InflateStream(\LaraGram\Http\Client\Core\PsrUtils::streamFor($stream));
    ...
    private function drain(StreamInterface $source, StreamInterface $sink, string $contentLength): StreamInterface
    {
        ...
        \LaraGram\Http\Client\Core\PsrUtils::copyToStream(
            $source,
            $sink,
