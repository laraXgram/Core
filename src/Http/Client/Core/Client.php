...
use LaraGram\Http\Client\Core\Cookie\CookieJarInterface;
use LaraGram\Http\Client\Core\Exceptions\Exception;
use LaraGram\Http\Client\Core\Exceptions\InvalidArgumentException;
use LaraGram\Http\Client\Core\Handler\CurlShareHandleState;
use LaraGram\Http\Client\Promises as P;
...
class Client implements ClientInterface, PsrClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = PsrUtils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($droppedHeaderNames !== [] && isset($options['_conditional'])) {
            $options['_conditional'] = PsrUtils::caselessRemove($droppedHeaderNames, $options['_conditional']);
        }
        $body = $options['body'] ?? null;
        $version = self::normalizeProtocolVersion($options['version'] ?? '1.1');
        // Merge the URI into the base URI.
        $uri = $this->buildUri(PsrUtils::uriFor($uri), $options);
        if (\is_array($body)) {
            throw $this->invalidBody();
        }
        $body = self::createBodyStream($body);
        $request = new \LaraGram\Http\Client\Core\Request($method, $uri, $headers, $body, $version);
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = UriResolver::resolve(PsrUtils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            ...
            $request = PsrUtils::modifyRequest($request, ['version' => '1.1']);
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = PsrUtils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (isset($options['decode_content']) && \is_string($options['decode_content'])) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = PsrUtils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = PsrUtils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = PsrUtils::modifyRequest($request, $modify);
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            ...
            return PsrUtils::streamFor(self::stringifyScalar($body));
        }

        if (\is_object($body) && \method_exists($body, '__toString')) {
            return PsrUtils::streamFor((string) $body);
        }
