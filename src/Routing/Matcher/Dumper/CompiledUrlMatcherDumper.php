...
class CompiledUrlMatcherDumper extends MatcherDumper
{
    ...
    public function dump(array $options = []): string
    {
        return <<<EOF
             ...
             * by the LaraGram Routing Component.
             */
    ...
    public function getCompiledRoutes(bool $forDump = false): array
    {
        ...
        if ($matchHost) {
            $compiledRoutes = [true];
            $routes = $routes->populateCollection(new BaseRouteCollection());
        } else {
     ...
     */
    private function groupStaticRoutes(BaseRouteCollection $collection): array
    {
        $staticRoutes = $dynamicRegex = [];
        $dynamicRoutes = new BaseRouteCollection();
     ...
     */
    private function compileDynamicRoutes(BaseRouteCollection $collection, bool $matchHost, int $chunkLimit, array &$conditions): array
    {
        if (!$collection->all()) {
            return [[], [], ''];
        }
        ...
        foreach ($collection->all() as $name => $route) {
            ...
            if ($chunkLimit < ++$chunkSize || $prev !== $rx[0] && $route->compile()->getPathVariables()) {
                $chunkSize = 1;
                $routes = new BaseRouteCollection();
     ...
     */
    private function compileRoute(BaseRoute $route, string $name, string|array|null $vars, bool $hasTrailingSlash, bool $hasTrailingVar, array &$conditions): array
    {
    ...
    public static function export(mixed $value): string
    {
        ...
        if (\is_object($value)) {
            throw new \InvalidArgumentException(\sprintf('LaraGram\Routing\BaseRoute cannot contain objects, but "%s" given.', get_debug_type($value)));
        }
        if (!\is_array($value)) {
